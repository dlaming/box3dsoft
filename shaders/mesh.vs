...
void main()
{
    ...
    vec4 viewPos = view * v_instanceMatrix * vec4(v_localPosition, 1.0);
    fragmentPosition = viewPos.xyz; 
    
    mat3 normalMatrix = transpose(inverse(mat3(view * v_instanceMatrix)));
    normal = normalMatrix * v_localNormal;
    
    int colorIndex = max(0, v_material) % 10;

    // alpha is specular
    color = v_material >= 0 ? vec4(colors[colorIndex], 1.0) : v_instanceColor;

    //color = vec4(colors[4], 1.0f);
