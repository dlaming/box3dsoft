...
UniqueMesh CreateUniqueMesh( const b3Vec3* vertexPositions, const b3Vec3* vertexNormals, int triangleCount,
							 const int* triangleIndices, const Edge* edges, int edgeCount, const int* materials, Arena arena )
{
	UniqueMesh shape = {};
	...
	// Vertex buffers
	{
		...
		glGenVertexArrays( 1, &shape.vertexArrayId );
		glGenBuffers( 1, &shape.vertexBufferId );

		// Copy static vertex data
		glBindVertexArray( shape.vertexArrayId );
	...
	// Edge buffer
	{
		...
		glGenVertexArrays( 1, &shape.edgeArrayId );
		glGenBuffers( 1, &shape.edgeBufferId );

		glBindVertexArray( shape.edgeArrayId );
		glBindBuffer( GL_ARRAY_BUFFER, shape.edgeBufferId );
		glBufferData( GL_ARRAY_BUFFER, 2 * edgeCount * sizeof( EdgeData ), edgeBuffer, GL_STATIC_DRAW );
...
{
	if ( shape->vertexBufferId != 0 )
	{
		glDeleteBuffers( 1, &shape->vertexBufferId );
	}

	if ( shape->vertexArrayId != 0 )
	{
		glDeleteVertexArrays( 1, &shape->vertexArrayId );
	}

	if ( shape->edgeBufferId != 0 )
	{
		glDeleteBuffers( 1, &shape->edgeBufferId );
	}

	if ( shape->edgeArrayId != 0 )
	{
		glDeleteVertexArrays( 1, &shape->edgeArrayId );
	}
}
